...
export default function AllAirbnb() {
  const [airbnb, setAirbnb] = useState<Accommodation[]>([]);
  const [currentPage, setCurrentPage] = useState(1);
  const [itemsPerPage, setItemsPerPage] = useState(8);
  const lastItemIndex = currentPage * itemsPerPage;
  const firstItemIndex = lastItemIndex - itemsPerPage;
  const currentItems = airbnb.slice(firstItemIndex, lastItemIndex);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`${API_URL}/type/airbnb`);
        console.log("Response from API:", response.data); // Log the API response
        if (response.data && Array.isArray(response.data.data)) {
          setAirbnb(response.data.data);
        } else {
        ...
        <div className="flex flex-col gap-8  w-full">
          <div className="  grid gap-8 xl:grid-cols-3 lg:grid-cols-3 md:grid-cols-2 justify-center">
            {airbnb.map((airbnb) => (
              ...
              >
                <Link
                  href={`/where-to-stay/airbnb/${airbnb.slug}`}
                  className=""
                >
                  <div className="w-full h-64 bg-green-400 relative overflow-hidden">
                    ...
                    >
                      <motion.div
                        ...
                        transition={{ duration: 0.3 }}
                      >
                        {airbnb.imageUrls && airbnb.imageUrls.length > 0 ? (
                          <Image
                            src={airbnb.imageUrls[0]}
                            alt={airbnb.name}
                            layout="fill"
                ...
                <div className="pt-2 flex flex-col justify-between h-32 gap-2">
                  <div className="flex flex-col gap-2 ">
                    <Link href={`/where-to-stay/airbnb/${airbnb.slug}`}>
