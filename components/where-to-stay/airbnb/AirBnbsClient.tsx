...
interface Data {
  airbnbs: Accommodation[];
}

export default function AllAirBnbsClient({ data }: { data: Data }) {
  const [airbnbs, setAirbnbs] = useState<Accommodation[]>([]);
  const [currentPage, setCurrentPage] = useState(1);
  const [itemsPerPage, setItemsPerPage] = useState(8);
  const lastItemIndex = currentPage * itemsPerPage;
  const firstItemIndex = lastItemIndex - itemsPerPage;
  const currentItems = airbnbs.slice(firstItemIndex, lastItemIndex);
  return (
    <>
              ...
              >
                ...
                <div className="pt-2 flex flex-col justify-between h-32 gap-2">
                  <div className="flex flex-col gap-2 ">
                    <Link href={`/where-to-stay/airbnbs/${airbnb.slug}`}>
