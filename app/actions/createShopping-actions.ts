...
import { CategoryType, ShoppingType } from "@prisma/client";
...
const shoppingSchema = z.object({
  ...
  type: z.nativeEnum(ShoppingType),
...
export async function createShopping(formData: FormData) {
  ...
  const data = shoppingSchema.parse({
  ...
  const uploadImageToSupabase = async (file: File): Promise<string | null> => {
    try {
      ...
      const uniquePath = `shopping/${Date.now()}-${sanitizedFileName}`;

      const { data, error } = await supabase.storage
        .from("shopping") // Ensure this matches your bucket name in Supabase
      ...
      return `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/shopping/${data.path}`;
    } catch (err) {
  ...
  revalidatePath("/what-to-do/shopping");
}
