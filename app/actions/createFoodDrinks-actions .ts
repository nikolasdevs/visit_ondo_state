...
const food_drinkSchema = z.object({
...
export async function createFoodDrink(formData: FormData) {
  ...
  const data = food_drinkSchema.parse({
  ...
  const uploadImageToSupabase = async (file: File): Promise<string | null> => {
    try {
      ...
      const uniquePath = `food_drink/${Date.now()}-${sanitizedFileName}`;

      const { data, error } = await supabase.storage
        .from("food_drink") // Ensure this matches your bucket name in Supabase
      ...
      return `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/food_drink/${data.path}`;
    } catch (err) {
  ...
  await prisma.food_Drink.create({
    data: {
      title: data.title,
